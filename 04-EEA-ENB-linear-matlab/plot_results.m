...
%% Frequency deviation from nominal value (x2)
    ...
    xlim([0 N])
    ylim([1.05*min(min(x(2:5:end,1:end))) 1.05*max(max(x(2:5:end,1:end)))])
    grid on
    hold on
        for i = 1:Na
            stairs(0:N,x(5*(i-1)+2,1:end),'-','LineWidth',1.5, 'Color', plotColors(i,:))
        end
...
%% Energy stored in the batteries (x3)
    ...
    xlim([0 N])
    ylim([1.05*min(min(x(3:5:end,1:end))) 1.05*max(max(x(3:5:end,1:end)/30))])
    grid on
    hold on
        for i = 1:Na
            stairs(0:N,x(5*(i-1)+3,1:end)/30, '-', 'LineWidth', 1.5, 'Color', plotColors(i,:))
        end
...
%% Cost Function
    ...
    hold on
%         for i = 1:Na
            stairs(0:N-1,abs(JVect(1,1:end)),'-','LineWidth',1.5)
%         end
...
%% Export Procedure
    ...
    if export_output_plot == true
        exportgraphics(f1,['SimulationOutput/',simID,'_X1.png'],'Resolution',600)
        exportgraphics(f2,['SimulationOutput/',simID,'_X2.png'],'Resolution',600)
        exportgraphics(f3,['SimulationOutput/',simID,'_X3.png'],'Resolution',600)
        exportgraphics(f11,['SimulationOutput/',simID,'_X4.png'],'Resolution',600)
        exportgraphics(f12,['SimulationOutput/',simID,'_X5.png'],'Resolution',600)
        exportgraphics(f4,['SimulationOutput/',simID,'_U1.png'],'Resolution',600)
        exportgraphics(f5,['SimulationOutput/',simID,'_U2.png'],'Resolution',600)
        exportgraphics(f13,['SimulationOutput/',simID,'_U3.png'],'Resolution',600)
        exportgraphics(f6,['SimulationOutput/',simID,'_W1.png'],'Resolution',600)
        exportgraphics(f7,['SimulationOutput/',simID,'_W2.png'],'Resolution',600)
        % exportgraphics(f8,['SimulationOutput/',simID,'_Topology.png'])
        exportgraphics(f9,['SimulationOutput/',simID,'_Ptie.png'],'Resolution',600)
        exportgraphics(f10,['SimulationOutput/',simID,'_Cost.png'],'Resolution',600)
        % exportgraphics(f14,['SimulationOutput/',simID,'_PlotLegend.png'],'Resolution',600)
    end
