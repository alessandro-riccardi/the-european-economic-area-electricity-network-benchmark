...
# %%
Simulation_ID = "25_EAA_DMPC_SimulationData"
...
residual_evolution = Simulation_Data[23][0]
...
###################################################################### Residual behavior
Fig_Residual, Ax_Residual = plt.subplots(figsize=(9, 6), dpi=100)

Ax_Residual.margins(x=0, y=0)
Ax_Residual.grid(True, color='gray', alpha=0.2)

Ax_Residual.set_title('Avarage residual behavior', fontsize=26)
Ax_Residual.set_xlabel('Time step')
Ax_Residual.set_ylabel('r(k)')

Ax_Residual.step(range(0,SIMULATION_HORIZON-1), 
                residual_evolution[0:SIMULATION_HORIZON], 
                where='pre', 
                )
...
Fig_Residual.savefig(OUTPUT_FOLDER + "/" + Simulation_ID + "_Fig_Residual.pdf", format="pdf", dpi=300, bbox_inches='tight')
