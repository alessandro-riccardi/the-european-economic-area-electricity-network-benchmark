...
# Augmented_Control_Agents = create_empty_list(NUMBER_CONTROL_AGENTS)
...
for l in range(0,NUMBER_CONTROL_AGENTS):
    ...
    B_Augmented = np.zeros((NUM_STATES*Size_Augmented_Control_Agent,2*Size_Augmented_Control_Agent))
    ...
    R_Augmented = np.zeros((Size_Augmented_Control_Agent,Size_Augmented_Control_Agent))
    ...
    for i in range(0, Size_Augmented_Control_Agent):
        ...
        for j in range(0, Size_Augmented_Control_Agent):
            ...
            A_Augmented[i*NUM_STATES:(i+1)*NUM_STATES,j*NUM_STATES:(j+1)*NUM_STATES] = A_MLD[Agent_Index_1*NUM_STATES:(Agent_Index_1+1)*NUM_STATES,Agent_Index_2*NUM_STATES:(Agent_Index_2+1)*NUM_STATES].copy()
            B_Augmented[i*NUM_STATES:(i+1)*NUM_STATES,j*NUM_INPUTS:(j+1)*NUM_INPUTS] = Bu_MLD_Nominal[Agent_Index_1*NUM_STATES:(Agent_Index_1+1)*NUM_STATES, Agent_Index_2*NUM_INPUTS:(Agent_Index_2+1)*NUM_INPUTS].copy()
            ...
            D_Augmented[i*NUM_STATES:(i+1)*NUM_STATES,j*2:(j+1)*2] = D_MLD[Agent_Index_1*NUM_STATES:(Agent_Index_1+1)*NUM_STATES,Agent_Index_2*2:(Agent_Index_2+1)*2].copy()
    ...
    Control_Agents_Matrices[l][1] = Bu_Augmented.copy()
...
for l in range(0,NUMBER_CONTROL_AGENTS):
    ...
    B_Augmented = Control_Agents_Matrices[l][6].copy()
    ...
    for i in range(1,PREDICTION_HORIZON+1):
        ...
        for j in reversed(range(0,i)):
            ...
            Cu_Optimization[Row_Index:Row_Index+NUM_STATES*Size_Augmented_Control_Agent,Col_Index_2:Col_Index_2+Size_Augmented_Control_Agent] = np.linalg.matrix_power(A_Augmented,j) @ Bu_Augmented
    ...
    Control_Agents_Optimization_Matrices[l][1] = Cu_Optimization.copy()
    ...
    R_Optimization = np.zeros((Size_Augmented_Control_Agent*PREDICTION_HORIZON,Size_Augmented_Control_Agent*PREDICTION_HORIZON), dtype=np.float64)
    ...
    R_Optimization = np.zeros((PREDICTION_HORIZON*Size_Augmented_Control_Agent,PREDICTION_HORIZON*Size_Augmented_Control_Agent))
...
for k in range(0,SIMULATION_HORIZON):
    ...
    while (number_of_iterations < max_number_of_iterations) and (norm_codition_1 == False) and (norm_codition_2 == False):
        ...
        for l in range(0,len(Augmented_Control_Agents)):
            ...
            C_Optimization = Control_Agents_Optimization_Matrices[l][6].copy()
            ...
            Lower_Bounds_u = np.zeros((number_of_agents_l*PREDICTION_HORIZON,1))
            Upper_Bounds_u = np.zeros((number_of_agents_l*PREDICTION_HORIZON,1))
            ...
            # Variables
            u_tilde_k = gurobi_model.addMVar((NUM_INPUTS*number_of_agents_l*PREDICTION_HORIZON, 1), lb = Lower_Bounds_u_PWA, ub = Upper_Bounds_u_PWA)
            ...
            x_tilde_k = M_Optimization @ x_k_l + Cu_Optimization @ u_tilde_k_Pdis + Cz_Optimization @ z_tilde_k + D_Optimization @ external_signals_k_l
            ...
            Hu_Optimization = (np.transpose(Cu_Optimization) @ Q_Optimization @ Cu_Optimization) + R_Optimization
            ...
            Stage_Cost = (u_tilde_k_Pdis.T @ Hu_Optimization @ u_tilde_k_Pdis) + (z_tilde_k.T @ Hz_Optimization @ z_tilde_k) + 2*(z_tilde_k.T @ np.transpose(Cz_Optimization) @ Q_Optimization @ Cu_Optimization @ u_tilde_k_Pdis) + 2*(np.transpose(x_k_l) @ np.transpose(M_Optimization) - reference_k_l.T) @ Q_Optimization @ Cu_Optimization @ u_tilde_k_Pdis + 2*(np.transpose(x_k_l) @ np.transpose(M_Optimization) - reference_k_l.T) @ Q_Optimization @ Cz_Optimization @ z_tilde_k + 2*(np.transpose(external_signals_k_l) @ np.transpose(D_Optimization)) @ Q_Optimization @ Cu_Optimization @ u_tilde_k_Pdis + 2*(np.transpose(external_signals_k_l) @ np.transpose(D_Optimization)) @ Q_Optimization @ Cz_Optimization @ z_tilde_k + np.transpose(gamma_l)@(consensus_variable_l - avarage_variable_l) + (rho/2)*np.transpose(consensus_variable_l - avarage_variable_l)@(consensus_variable_l - avarage_variable_l)
            ...
            PESS_local_k = np.zeros((number_of_agents_l, PREDICTION_HORIZON))
            ...
            for k_local in range(0,PREDICTION_HORIZON):
                x_local_k[:,k_local+1] = A_Augmented @ x_local_k[:,k_local] + Bu_Augmented @ PDisp_local_k[:,k_local] + Bz_Augmented @ z_local_k[:,k_local] + D_Augmented @ d_local_k[:,k_local]
            ...
            for i in range(0,NUMBER_CONTROL_AGENTS):
                ...
                for j in range(0,len(Augmented_Control_Agents[i])):
                    if l == Augmented_Control_Agents[i][j]:
                        u_avarage_local[l*2:(l+1)*2,:] = u_avarage_local[l*2:(l+1)*2,:] + u_local_k[j*2:(j+1)*2,:]
                        x_avarage_local[l*5:(l+1)*5,:] = x_avarage_local[l*5:(l+1)*5,:] + x_local_k[j*5:(j+1)*5,:]
                        total_copies += 1
                Total_Time_Holder = tm.time() - Start_Time_Holder
                Core_Seconds[k][i,number_of_iterations] = Core_Seconds[k][i,number_of_iterations] + Total_Time_Holder
            if total_copies > 0:        
                u_avarage_local[l*2:(l+1)*2,:] = u_avarage_local[l*2:(l+1)*2,:]/total_copies
                x_avarage_local[l*5:(l+1)*5,:] = x_avarage_local[l*5:(l+1)*5,:]/total_copies
        ...
        for l in range(0,NUMBER_CONTROL_AGENTS):
            ...
            for i in range(0,number_of_agents_l):
                ...
                for Input_Index in range(0,2):
                    y_avarage = np.concatenate((y_avarage,u_avarage_local[(variable_index*2)+Input_Index,:]), axis=None)
            for i in range(0,number_of_agents_l):
                variable_index = LIST_ATOMIC_AGENTS.index(Augmented_Control_Agents[l][i])
                for Input_Index in range(0,5):
                    y_avarage = np.concatenate((y_avarage,x_avarage_local[(variable_index*5)+Input_Index,:]), axis=None)
    ...
    B_Dynamics_Nominal = PARAMETERS_LIST[4].copy()
...
if DRAW_PLOTS == True:
    ...
    Plot_Input_02.update_layout(title="Power exchange with ESS", title_x=0.5, 
...
# %%
# MARK: Export Data
Simulation_Data = [[PARAMETERS_LIST],[Eta],[MEASURED_LOAD],[MEASURED_RENEWABLE],[MEASURED_LOAD_INCREMENTS],[MEASURED_RENEWABLE_INCREMENTS],[INITIAL_DISPATCHABLE_POWER],[Control_Agents],[Augmented_Control_Agents],[Control_Agents_Matrices],[Control_Agents_Optimization_Matrices],[Weighted_Adjacency_Matrix],[Reference_Signal],[u_evolution],[x_evolution],[w_evolution],[z_evolution],[d_evolution],[Simulation_Total_Time],[Total_Computation_Time],[Total_Core_Seconds],[Total_Number_ADMM_Iterations],[Cost_Function],[residual_evolution]]
