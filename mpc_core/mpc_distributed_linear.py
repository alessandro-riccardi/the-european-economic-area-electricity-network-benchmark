...
def control_simulation_linear_distributed_mpc(simulation_data):
    ...
    for i in range(0,NUMBER_ATOMIC_AGENTS):
        ...
        if(INITIAL_DISPATCHABLE_POWER[Agent_Idx] >= 3*UPPER_BOUNDS_STATE[4,i]/4):
            Initial_Dispatchable_Power_Processed[i,0] = 3*UPPER_BOUNDS_STATE[4,i]/4
        else:
            Initial_Dispatchable_Power_Processed[i,0] = INITIAL_DISPATCHABLE_POWER[Agent_Idx]
        
    #  Initial state for the dipatchable generation
    # for i in range(0,NUMBER_ATOMIC_AGENTS):
    #     x_evolution[(i*NUM_STATES)+4,0] = Initial_Dispatchable_Power_Processed[i,0]

    # Set initial state system evolution
    x_evolution[:,0] = EEA_ENB.x.copy().reshape(-1)
