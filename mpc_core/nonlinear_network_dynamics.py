...
class NonlinearNetworkDynamics:
    ...
    def __init__(self, atomic_agents, Weighted_Adjacency_Matrix, control_time_step, intertia_modifiers, INITIAL_DISPATCHABLE_POWER):
        ...
        B_Dynamics_Nominal = np.zeros((self.NUM_STATES*self.NUMBER_ATOMIC_AGENTS,self.NUM_INPUTS*self.NUMBER_ATOMIC_AGENTS))
        ...
        for i in range(0,self.NUMBER_ATOMIC_AGENTS):
            ...
            B_Dynamics_Nominal[i*self.NUM_STATES:(i+1)*self.NUM_STATES,i*self.NUM_INPUTS:(i+1)*self.NUM_INPUTS] = B_i_nominal.copy()
