...
import time as tm
from mpc_core.mpc_centralized_linear import control_simulation_linear_centralized_mpc
from mpc_core.mpc_distributed_linear import control_simulation_linear_distributed_mpc
...
def control_simulation(simulation_data):
    ...
    partitioning_strategy = simulation_data["partitioning_strategy"]

    if model == 'linear' and control_strategy == 'Centralized_MPC':
        if control_strategy == "Centralized_MPC" and partitioning_strategy != "centralized":
            raise ValueError("The partitioning strategy must be 'centralized' when the control strategy is 'Centralized_MPC'.")
        else: 
            return control_simulation_linear_centralized_mpc(simulation_data)
    elif model == 'linear' and control_strategy == 'Distributed_MPC':
        if control_strategy == "Distributed_MPC" and partitioning_strategy == "centralized":
            raise ValueError("The partitioning strategy must be non-centralized when the control strategy is 'Distributed_MPC'.")
        else:
            return control_simulation_linear_distributed_mpc(simulation_data)
    else:
